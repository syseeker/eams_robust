        ...
        for m in xrange(self.NUM_MEETING_TYPE):  
            ...
            for k in xrange(self.NUM_SLOT):
                if (self.EAMS.isInOngoingTimeWindows(mid, self.CURR_K+k) > 0 and
                    ...
                    name = '_'.join(name)         
                    self.CAV_T_MSLACK_LB_MK[m].append(self.model.addVar(lb=0.0, ub=self.FIXED_SLACK, vtype=GRB.CONTINUOUS, name=name))
                    ...
                    name = '_'.join(name)         
                    self.CAV_T_MSLACK_UB_MK[m].append(self.model.addVar(lb=0.0, ub=self.FIXED_SLACK, vtype=GRB.CONTINUOUS, name=name))
        ...
        for mtype_id, v in self.TMP_DICT.iteritems():
            if self.SCHE_MODE == 0 or (self.SCHE_MODE == 1 and v[0] in self.CURR_DESTROY_LOCATION):
                ...
                for dl, dk in v:
                    ...
                    name = '_'.join(name)                  
                    self.CAV_T_MSLACK_LB_PREVSES_MK[m].append(self.model.addVar(lb=0.0, ub=self.FIXED_SLACK, vtype=GRB.CONTINUOUS, name=name))
                     
                    name = ['CAV_T_MSLACK_UB_MK', str(mtype_id), str(dk)]                            
                    name = '_'.join(name)                  
                    self.CAV_T_MSLACK_UB_PREVSES_MK[m].append(self.model.addVar(lb=0.0, ub=self.FIXED_SLACK, vtype=GRB.CONTINUOUS, name=name))
...
# 2.645751311        0.99
# 2.828427125        1.00
    def _calc_slack(self, dm):
        delta = -1
        a_bar = 0
        a_hat = 0.5
        
        if dm == 1:
            return self.PROB_GUAR * a_hat
            
        if self.PROB_GUAR == 0.75:
            delta = 1.414213562
        elif self.PROB_GUAR == 0.5:
            delta = 1            
        else:
            logging.critical("Unsupported prob. guarantee")
            return 0
            
        ret = self.procedure(delta, dm*[a_bar], dm*[a_hat])
        if ret == -1:
...
#         print S
    ...
    def _createCSTR_MeetingBasedRoomTemperature_Slack(self):
        ...
        for m in xrange(len(self.CAV_T_MSLACK_LB_MK)):
            if self.SCHE_MODE == 1:
                mtype_id = self.CURR_DESTROY_MTYPE[m]
                mdur = self.EAMS.ML[mtype_id].Duration
            else:
                mtype_id = self.MTYPE_ID[m]
                mdur = self.EAMS.ML[mtype_id].Duration
            
            if self.PROB_GUAR > 0:
                slack = self._calc_slack(mdur)
            else:
                slack = 0
            logging.info("Meeting duration: %s, slack: %s" %(mdur, slack))
            ...
            name = '_'.join(name)
            self.CSTR_T_SLACK_M.append(self.model.addConstr(lcstr + slack <= self.DELTA, name))
        ...
        for m in xrange(len(self.CAV_T_MSLACK_LB_PREVSES_MK)):            
            ...
            if self.SCHE_MODE == 0 or (self.SCHE_MODE == 1 and sl in self.CURR_DESTROY_LOCATION):
                ...
                if sk < self.CURR_K:
                    ...
                    rcstr = self.DELTA - used_slack_lb - used_slack_ub
                else:
                    rcstr = self.DELTA
                ...
                name = ['CSTR_T_SLACK_M', str(mtype_id)]
                name = '_'.join(name)
                self.CSTR_T_SLACK_PREVSES_M.append(self.model.addConstr(lcstr + slack <= rcstr, name))
        ...
        for l in xrange(self.NUM_ROOM):
            ...
            for k in xrange(self.NUM_SLOT):
                ...
                if self.MSTR_SCHE.MSTR_BAV_z_LK_MeetingOccupiedMap_Reverse.get(tuple([dl,self.CURR_K+k])) is not None:
                    rcstr = self.FIXED_SLACK    # Just set {lk} to Tu if it is occupied
                # For new meeting
                else:
                    for m in xrange(self.NUM_MEETING_TYPE):
                        if self.SCHE_MODE == 1:
                            ...
                            mtype_id = self.MTYPE_ID[m]
                            mid = self.MTYPE[m].MLS[0]
                        xctr += self.get_x(dl, self.CURR_K+k, mid)
                    rcstr = self.FIXED_SLACK * xctr
